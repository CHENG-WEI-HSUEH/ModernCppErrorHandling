...
static std::filesystem::path make_file_with(const std::filesystem::path& dir,const std::string& filename,const std::string& content)
{
...
TEST_F(ErrorCasesTest, ReadMissingFile_Triggers_ConfigReadError) 
{
    ...
    // 回傳 unexpected(ConfigReadError)  :contentReference[oaicite:2]{index=2}
    auto res = LoadConfig(path.string()); 
...
TEST_F(ErrorCasesTest, InvalidField_Triggers_ValidationError) 
{
    ...
    auto res = LoadConfig(p.string())
        // 期望在此步驟失敗  :contentReference[oaicite:4]{index=4}
...
{
    // 刻意過短
    ValidatedData very_short{ "x" }; 
    // 預期為 ProcessingError（資料太短）  :contentReference[oaicite:6]{index=6}
    auto res = ProcessData(very_short); 
